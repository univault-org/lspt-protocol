...
#include <stdexcept>
#include "../../src/core/srpt_handshake.h"
...
TEST(HandshakeTest, GenericHandshake) {
    ...
    EXPECT_EQ(handshake1.getState(), SRPT::HandshakeState::Established);
    EXPECT_EQ(handshake2.getState(), SRPT::HandshakeState::Established);
}
...
TEST(HandshakeTest, ExceptionOnUnexpectedMessage) {
    ...
    handshake1.handleHandshakeMessage(SRPT::Common::ByteVector{1});
    handshake2.handleHandshakeMessage(SRPT::Common::ByteVector{1});
